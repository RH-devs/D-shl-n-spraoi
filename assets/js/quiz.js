...
options.forEach(option => {
	option.addEventListener('click', e => {
		...
		let optionToApply = selectedResult == currentQuestion.correctAnswer ? 'rightOption' : 'wrongOption'
          ...
          selectedOption.parentElement.classList.add(optionToApply);// Code adds the correct answer to the parent element when the user clicks on the correct answer.
		setTimeout(() => {
     ...
     if (finalScoreCounter > 500 && finalScoreCounter <= 1000) {
                   goodCredit.innerText = `Good Job Fam😊!!! Your Total score is ${score} out of ${totalScoreofPercentGrade}, You are an Executive!`;
     } else { //less than or equal to 500
                   badCredit.innerText = `Awwww, you lost Fam!!! Your Total score is ${score} out of ${totalScoreofPercentGrade}, You can be Better Next Time!`;
     }
